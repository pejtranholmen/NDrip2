...
bool NC_Plant_Functions::Def()
{
	...
	for (size_t jj = 0; jj < NUM_BIOELEM - 1; jj++) {
		...
		Define(pG, pNumPlants, P_BioElementName[jj] + "Plant LitterFall", initv, MASSFLOW_UNIT, "Nitrogen and Carbon >= 1|Growth >0" + conditions);
	...
	Define(pP, pNumPlants, "Mulch", initv, "-", "WaterEq = 1|Evaporation Method > 0", 0.5, 20.);
...
bool NC_Plant_Functions::Def_GrowthStages()
{
	...
	Define(pP, pNumPlants, "C Seed", initv, MASS_UNIT, "Nitrogen and Carbon >= 1|Sowing>0", 0.01, 1000.);
	mTab_Start.push_back(pP);
	...
	Define(pP, pNumPlants, "Initial Tsum", initv, TEMPSUM_UNIT, "Nitrogen and Carbon >= 1|Growth >= 2", 0., 1500.);
...
bool NC_Plant_Functions::Def_PhotoSynthesis()
{
	...
	Define(pP, pNumPlants, "Vcmax", initv, "micro mol/m2/s", "Nitrogen and Carbon >= 1|Growth>3", 10., 400.);
	mTab_Farq.push_back(pP);
...
Photo NC_Plant_Functions:: PhoSib(size_t index) {
	...
	NC_Organic* pNCSoil=&p_Soil->m_NC_SoilOrg;
		...
		if (RadInterception >= 1)
			Vmax[index] = 1.E-6*Vcmax[index] * pPlant->LAI[index]*(1 - exp(-pAtm->RntLAIv[index] * pPlant->LAI[index]));	// Enhet mol/m2/s
		else
			Vmax[index]= 1.E-6*Vcmax[index]* pPlant->SimLeafAreaIndex[index]*(1-exp(-pAtm->RntLAIv[index]*pPlant->SimLeafAreaIndex[index]));	// Enhet mol/m2/s
	
	   
	   	VmaxRubisco[index]=Vmax[index]*min(GPP_Current.Temp[index],GPP_Current.Water[index]);		
...
// Not to be converted here			/MOL_C/86400)
			// Dessa är nu per 
			if (ASSIMNY[index][ICount] < 0.0000001) {
				EYY[index][ICount] = 0.;
			}
			else {
...
//		i bladverket. De är viktiga för nästa tidssteg.
		...
		if(C3Type[index]>0) {	
			...
			out.Sink=P_SinkLimitedC3(Vmax[index])*10e6;
				...
				out.Sink=P_SinkLimitedC4_CO2(P_Sink1, PCo2y[index][ICConv[index]])*10;
		}
		return out;
