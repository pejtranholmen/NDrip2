...
class Sw : public SimB {
 public:
	...
	~Sw();
	void Def(size_t module_no, std::string varname, int initv, std::string options, std::string conditions, int recalc);  
public:
	...
	std::string GetOption(size_t index) const {if(index<_options.size()) return _options[index]; else return ""; };
	size_t GetNumOptions() const {return _options.size();};
	bool IsOldValue() {return _IsOldValue;};
	size_t SetStrValue(std::string str);
	size_t SetDBStrValue(std::string str);
